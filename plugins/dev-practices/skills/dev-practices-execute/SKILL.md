...
description: >
  ...
  "cook formula", "execute beads tasks", "task execution", "bug triage",
  "run task pipeline", "walk the molecule", "atom-based execution". Auto-selects
  formula based on task type: bug tasks use bug-triage, tasks needing
  research/design use task-execute, well-defined tasks use task-single. All
  formulas enforce quality gates and test integrity. Findings stored in beads
  (not filesystem).
...
```
/dev-practices-execute <task-id-1> [task-id-2] [task-id-3] ...
```
...
production code to gate on. The execute atom runs the project quality command as the gate.
...
Formulas are in this skill's `formulas/` directory. Use `${CLAUDE_PLUGIN_ROOT}`
to resolve paths.

**For bugs** (`bd show` shows type=bug):
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/scripts/cook_formula.py \
  --formula ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/formulas/bug-triage.yaml \
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/scripts/cook_formula.py \
  --formula ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/formulas/task-single.yaml \
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/scripts/cook_formula.py \
  --formula ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/formulas/task-execute.yaml \
  --var "TASK_IDS={all_args}" \
  --epic-title "Execute: {all_args}"
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/scripts/cook_formula.py \
  --formula ${CLAUDE_PLUGIN_ROOT}/skills/dev-practices-execute/formulas/<formula>.yaml \
  --var "<VAR>={all_args}" \
  --epic-title "<title>" \
  --dry-run
```
...
The `formulas/` directory contains these templates:

| Formula | Purpose | Atoms/item |
|---------|---------|-----------|
| `task-single.yaml` | Well-defined tasks | 3 |
| `task-execute.yaml` | Complex tasks with TDD | 7 |
| `task-fix.yaml` | Code fixes with separate verify gate | 4 |
| `bug-triage.yaml` | Bug reproduction and fix | 7 |
| `structural-guard.yaml` | Create AST-scanning guard tests | 6 |
| `integration-from-stub.yaml` | Derive integration tests from stubs | 8 |
| `entity-remediate.yaml` | Fill test stubs in entity suites | 8 |
| `obligation-test.yaml` | Per-obligation test derivation | 4 |
| `migration-audit.yaml` | Repeatable code review audit | 4 (linear) |
| `bdd-step.yaml` | BDD step implementation | 6 |

## See Also

- `/dev-practices-remediate` -- Fill entity test stubs batch-by-batch with TDD
- `/dev-practices-obligation-test` -- Write per-obligation behavioral tests
- `/dev-practices-code-review` -- Multi-agent code review
